...
def w_batdata(num,hsname,hshp,hsmp,sp1,sp2,sp3,sp4,sp5,sp6,sp7,starttime,lasttime,endtime):
    conn = pymysql.connect(**dbinfo)
    cursor=conn.cursor()
    cursor.execute("insert into batdata values('"+str(num)+"','"+str(hsname)+"','"+str(hshp)+"','"+str(hsmp)+"','"+str(sp1)+"','"+str(sp2)+"','"+str(sp3)+"','"+str(sp4)+"','"+str(sp5)+"','"+str(sp6)+"','"+str(sp7)+"','"+str(starttime)+"','"+str(lasttime)+"','"+str(endtime)+"')")
    conn.commit()
    cursor.close()
    conn.close()
